...
var
   hProcesses, hOrgFiles, hFiles: array of THandle;
   hCurProcess: THandle;
   FileNames: aString;
   ProcessIDs, aAccess: array of DWORD;
...
begin
   ...
   try
      ...
      Index := Ord(pThreadParam.FileName[0]) - 1;
      pThreadParam.FileName[0] := #0;
   ...
   begin
      LastError := 0;
      try
         idDuplicated := DuplicateHandle(hProcesses[Index], hOrgFiles[Index], GetCurrentThread(), @hFiles[Index], 0, False, DUPLICATE_SAME_ACCESS);
...
function GetFileNameHandle: aString;
var
   i: Integer;
   lpExitCode: DWORD;
...
begin
   ...
   begin
      ...
      pThreadParams[i].FileName[0] := Char(i + 1);
      try
   ...
   for i := 0 to FnCPU - 1 do
      ...
      try
         ...
         case WaitForSingleObject(hThreads[i], 1000) of
            WAIT_OBJECT_0:
               begin
...
begin
   ...
   begin
      ...
      hCurProcess := OpenProcess(PROCESS_DUP_HANDLE or PROCESS_QUERY_INFORMATION, FALSE, CurProcessID);
      for aIndex := 0 to pHandleInfo^.uCount - 1 do
         if pHandleInfo.Handles[aIndex].ObjectType = nObjectType then //Files and folders
...
begin
   Result := 0;
   try
      hGetType := CreateFile('Nul', GENERIC_READ, 0, nil, CREATE_NEW, 0, 0);
   except
...
begin
   ...
   FnCPU := 1; //NumberOfProcessors;
   FhCPU := 0; //NumberOfProcessors - 1;
